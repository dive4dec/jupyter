...
static int verify_packet(radius_server_t *server, AUTH_HDR *response, AUTH_HDR *request, radius_conf_t *conf)
{
	...
	/*
	 * Message-Authenticator verification is disabled because the CityU NPS
	 * RADIUS servers send an incorrect HMAC. The Response Authenticator
	 * check above is sufficient to verify packet integrity.
	 */
	return TRUE;
}
